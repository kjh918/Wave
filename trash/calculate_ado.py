    ...
    for chrom in chrom_list:
        ...
        for bam_path in bam_list:
            ...
            if sample_id == 'cbNIPT_25_03_01':
                out_vcf = f'{work_dir_path}/{sample_id}/09_HaplotypeCall_BAM/{sample_id}.{chrom}.gvcf'
                os.system(f'bgzip {out_vcf}')
                os.system(f'tabix -p vcf {out_vcf}.gz')
                wgs_vcf = f'{out_vcf}.gz'
            else:
                out_vcf = f'{work_dir_path}/{sample_id}/09_HaplotypeCall_BAM/{sample_id}.{chrom}.gvcf'
                os.system(f'bgzip {out_vcf}')
                os.system(f'tabix -p vcf {out_vcf}.gz')
                cbnipt_vcf += f'{out_vcf}.gz '
            
        cmd = f'bcftools merge {wgs_vcf} {cbnipt_vcf} -Oz -o {work_dir_path}/merged.{chrom}.gvcf.gz'
        os.system(cmd)
