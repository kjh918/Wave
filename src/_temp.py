...
from src.tasks.task import TaskRegistry
from src.tasks.executor import Node, Executor
from src.pipeline.loader import render_template

# Placeholders supported:
#   - {sample_id}, {work_dir_path}
#   - {WORK_DIR}  (step-local)
#   - {STEP.<StepName>.<key>}  → reference a previous step's resolved field
#       e.g., inputs: { in_bam: "{STEP.Picard_Dedup.out_bam}" }
# Keys look up in *that step's* OUTPUT (then INPUT then PARAMS) dict.

STEP_REF = re.compile(r"\{STEP\.([A-Za-z0-9_\-]+)\.([A-Za-z0-9_\-]+)\}")


def _resolve_step_refs(mapping: Dict[str, Any], stash: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    def resolve_value(v: Any) -> Any:
        if isinstance(v, str):
            def repl(m: re.Match[str]) -> str:
                step_name, key = m.group(1), m.group(2)
                if step_name not in stash:
                    raise KeyError(f"Unknown step in reference: {step_name}")
                step_ctx = stash[step_name]
                if key in step_ctx.get("OUTPUT", {}):
                    return str(step_ctx["OUTPUT"][key])
                if key in step_ctx.get("INPUT", {}):
                    return str(step_ctx["INPUT"][key])
                if key in step_ctx.get("PARAMS", {}):
                    return str(step_ctx["PARAMS"][key])
                raise KeyError(f"Unknown key in STEP reference: {step_name}.{key}")
            return STEP_REF.sub(repl, v)
        elif isinstance(v, list):
...
class Workflow:
    """Sequential workflow that wires step I/O via placeholders and runs in order.

    Usage:
        wf = Workflow(sample_id="S01", work_dir_path="/work")
        wf.add_step(...)
        wf.run(dry_run=True)
    """

    def __init__(self, sample_id: str, work_dir_path: str | Path, default_params: Optional[Dict[str, Any]] = None):
        self.sample_id = sample_id
        self.work_dir_path = str(work_dir_path)
        self.default_params = default_params or {}
        self.steps: List[StepSpec] = []
        # keep a stash of resolved maps per step for {STEP.X.y} references
        self._resolved_stash: Dict[str, Dict[str, Dict[str, Any]]] = {}
    ...
    def add_step(self,
                 ...
                 inputs: Dict[str, Any],
                 outputs: Dict[str, Any],
                 params: Optional[Dict[str, Any]] = None) -> "Workflow":
        self.steps.append(
            StepSpec(name=name,
                     tool=tool,
                     func=func,
    ...
    # ---- resolution ----
    def _resolve_step(self, step: StepSpec) -> StepSpec:
        # 1) resolve WORK_DIR from {sample_id},{work_dir_path}
        task_ctx = {"sample_id": self.sample_id, "work_dir_path": self.work_dir_path}
        work_dir_str = render_template(step.work_dir, task_ctx)
        # 2) enrich context with {WORK_DIR}
        ctx = {**task_ctx, "WORK_DIR": work_dir_str}
        # 3) expand strings in I/O/PARAMS for simple placeholders
        inputs = {k: render_template(v, ctx) if isinstance(v, str) else v for k, v in step.inputs.items()}
        outputs = {k: render_template(v, ctx) if isinstance(v, str) else v for k, v in step.outputs.items()}
        params = {k: render_template(v, ctx) if isinstance(v, str) else v for k, v in step.params.items()}
        # 4) resolve inter-step references like {STEP.Prev.out_bam}
        # stash contains fully resolved maps for previous steps only
        resolved_maps = {"INPUT": inputs, "OUTPUT": outputs, "PARAMS": params}
        resolved_maps = _resolve_step_refs(resolved_maps, self._resolved_stash)
        # 5) update stash for later steps
        self._resolved_stash[step.name] = resolved_maps
        # 6) return a concrete StepSpec
        return StepSpec(
            name=step.name,
            tool=step.tool,
            func=step.func,
            work_dir=work_dir_str,
            inputs=resolved_maps["INPUT"],
            outputs=resolved_maps["OUTPUT"],
            params=resolved_maps["PARAMS"],
        )

    # ---- execution ----
    def compile(self) -> List[Node]:
        nodes: List[Node] = []
        self._resolved_stash.clear()
        for step in self.steps:
            rs = self._resolve_step(step)
            TaskClass = TaskRegistry.get(rs.tool)
            task = TaskClass(
                name=rs.name,
                tool=rs.tool,
                func=rs.func,
                threads=int(rs.params.get("threads", 1)),
                workdir=Path(rs.work_dir),
                inputs=rs.inputs,
                outputs=rs.outputs,
                params=rs.params,
            )
    ...
    def run(self, dry_run: bool = False) -> None:
        nodes = self.compile()
        Executor(dry_run=dry_run).run(nodes)

    # ---- export introspection (optional) ----
    def manifest(self) -> Dict[str, Any]:
        """Return a JSON-serializable manifest of the resolved workflow (after compile)."""
        _ = self.compile()  # refresh stash
        return {
            name: self._resolved_stash[name]
            for name in [s.name for s in self.steps]
        }


# ================================
# Example usage (binds Picard + FastQC in one pass)
# ================================
if __name__ == "__main__":
    wf = Workflow(sample_id="SAMPLE01", work_dir_path="/tmp/work")
    ...
    wf.add_step(
        ...
        inputs={
            "read1": "/storage/.../{sample_id}_R1.fastq.gz",
            "read2": "/storage/.../{sample_id}_R2.fastq.gz",
        },
        outputs={
    ...
    # 1) Picard MarkDuplicates (inputs may reference previous outputs with {STEP.<name>.<key>})
    wf.add_step(
        name="Picard_Dedup",
        tool="picard",
        func="markduplicates",
        work_dir="{work_dir_path}/{sample_id}/02_Dedup",
        inputs={
            "in_bam": "/data/bam/{sample_id}.sorted.bam",
        },
        outputs={
            "out_bam": "{WORK_DIR}/{sample_id}.dedup.bam",
            "metrics": "{WORK_DIR}/{sample_id}.dedup.metrics.txt",
    ...
    # 2) Downsample referencing previous step
    wf.add_step(
        ...
        work_dir="{work_dir_path}/{sample_id}/03_Downsample",
        inputs={
            "in_bam": "{STEP.Picard_Dedup.out_bam}",
        },
        outputs={
            "out_bam": "{WORK_DIR}/{sample_id}.ds0.5.bam",
        },
    ...
    # Dry-run to see the chained commands
    wf.run(dry_run=True)

# ================================
# src/pipeline/workflow_multitasks.py
# ================================
from __future__ import annotations
from dataclasses import dataclass
from pathlib import Path
...
from src.tasks.executor import Node, Executor
from src.pipeline.loader import render_template
from src.pipeline.workflow import StepSpec, _resolve_step_refs  # reuse utilities


@dataclass
class MultiTasks:
    """Group of steps executed sequentially under a named unit.

    The group exposes `Tasks` (capitalization as requested): a Python list of `StepSpec`.
    Later steps (inside or after the group) can reference earlier ones via
    `{STEP.<StepName>.<key>}` just like in the base workflow.
    """
    name: str
    Tasks: List[StepSpec]
    ...
    def add_step(self,
                 ...
                 inputs: Dict[str, Any],
                 outputs: Dict[str, Any],
                 params: Optional[Dict[str, Any]] = None) -> "MultiTasks":
        (self.Tasks or []).append(
            StepSpec(
                name=name,
                tool=tool,
                func=func,
...
class Workflow:
    """Workflow that supports single `StepSpec` and grouped `MultiTasks` items.

    - Use `add_step(...)` to append a single step
    - Use `add_multitasks(MultiTasks(...))` to append a group
    """
    ...
    def __init__(self, sample_id: str, work_dir_path: str | Path, default_params: Optional[Dict[str, Any]] = None):
        ...
        self.work_dir_path = str(work_dir_path)
        self.default_params = default_params or {}
        self._items: List[Union[StepSpec, MultiTasks]] = []
        self._resolved_stash: Dict[str, Dict[str, Dict[str, Any]]] = {}
    ...
    def add_multitasks(self, group: MultiTasks) -> "Workflow":
        # inject default_params into each sub step
        normalized_steps: List[StepSpec] = []
        for s in group.Tasks:
            normalized_steps.append(
                StepSpec(
                    ...
                    inputs=s.inputs,
                    outputs=s.outputs,
                    params={**self.default_params, **(s.params or {})},
                )
            )
        self._items.append(MultiTasks(group.name, normalized_steps))
        return self
    ...
    def compile(self) -> List[Node]:
        ...
        def compile_one(s: StepSpec):
            rs = self._resolve_step(s)
            TaskClass = TaskRegistry.get(rs.tool)
            task = TaskClass(
                name=rs.name,
                tool=rs.tool,
        ...
        for item in self._items:
            if isinstance(item, StepSpec):
                compile_one(item)
            elif isinstance(item, MultiTasks):
                for sub in item.Tasks:
                    compile_one(sub)
            else:
                raise TypeError(f"Unsupported item in workflow: {type(item)}")
        return nodes

    def run(self, dry_run: bool = False) -> None:
        Executor(dry_run=dry_run).run(self.compile())
    ...
    def manifest(self) -> Dict[str, Any]:
        ...
        for item in self._items:
            if isinstance(item, StepSpec):
                ...
                order.extend([s.name for s in item.Tasks])
        return {name: self._resolved_stash[name] for name in order}


# ================================
# Example of MultiTasks usage
# ================================
if __name__ == "__main__":
    # Build a group of raw-QC tasks
    qc_group = MultiTasks(name="Raw_QC", Tasks=[])
    qc_group.add_step(
        name="Rawdata_FastQC",
        tool="fastqc",
        func="run",
        ...
        params={"threads": 4, "extract": True},
    )

    wf = Workflow(sample_id="S01", work_dir_path="/tmp/work")
    wf.add_multitasks(qc_group)
    wf.add_step(
        name="Picard_Dedup",
        tool="picard",
        func="markduplicates",
        work_dir="{work_dir_path}/{sample_id}/02_Dedup",
        inputs={"in_bam": "/data/{sample_id}.sorted.bam"},
        outputs={"out_bam": "{WORK_DIR}/{sample_id}.dedup.bam"},
